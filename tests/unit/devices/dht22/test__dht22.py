...
def test_instantiation(dht22_sensor: DHT22Sensor, pigpio_pi: MagicMock) -> None:
    """Test that the class can be instantiated."""
    dht22_sensor = DHT22Sensor(pi_obj=pigpio_pi, gpio=4)

    assert pigpio_pi.set_pull_up_down.called_once_with(4, 0)
...
def test_trigger_method(dht22_sensor: DHT22Sensor, pigpio_pi: MagicMock) -> None:
    ...
    assert pigpio_pi.write.called_once_with(4, 1)
    assert pigpio_pi.set_mode.called_once_with(4, 0)
    assert pigpio_pi.set_watchdog.called_once_with(4, 200)
...
def test_cancel_method(dht22_sensor: DHT22Sensor, pigpio_pi: MagicMock) -> None:
    ...
    assert pigpio_pi.set_watchdog.called_once_with(4, 0)
    assert pigpio_pi.callback.cancel.called_once()
